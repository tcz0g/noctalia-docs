...
- **Blank screen** - Check greetd / journal logs (info → stdout, warn/error → stderr by default). If using `NOCTALIA_GREETER_LOG`, check that file. Ensure `/var/lib/noctalia-greeter` exists (`just setup-log-dir` from the greeter source tree, or create it owned by the greetd user).
